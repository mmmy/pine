...
本策略是一个基于纺锤线形态和阻力位突破的Pine Script交易策略，适用于TradingView平台。策略结合了K线形态分析和时间段过滤，为交易者提供自动化的开仓信号。

⚠️ **重要提醒**: 本策略仅提供开仓信号，**不包含平仓逻辑**。一旦开仓后不会再次开仓，需要手动平仓或修改策略开始时间重新开始。

## 策略特点

### 核心功能
- **双重开仓条件**: 阻力位突破 + 纺锤线信号
- **多时间段交易**: 支持4个可配置的交易时间段
...
| 参数名称 | 类型 | 默认值 | 说明 |
|---------|------|--------|------|
| **策略开始时间** | 时间 | 2024-01-01 00:00:00 | 策略开始执行的时间，只有在此时间之后才会开仓 |
| **交易方向** | 选择 | 做多 | 可选择"做多"或"做空" |
| **开仓数量** | 数值 | 1.0 | 每次开仓的数量，最小值0.01 |
...
| **时间段1-4** | 时间段 | 分别为08:00-12:00, 13:00-17:00, 19:00-23:00, 00:00-06:00 | 四个可配置的交易时间段 |
| **启用开关** | 开关 | 全部开启 | 每个时间段都有独立的启用开关 |
...
### 2. 参数配置
1. **设置策略开始时间**: 选择希望策略开始工作的时间点
2. **选择交易方向**: 根据市场判断选择做多或做空
3. **配置交易参数**: 设置开仓数量、阻力位价格等
4. **设置时间段**: 根据市场特点配置合适的交易时间
5. **调整显示选项**: 根据个人喜好开启相关显示功能

### 3. 监控和管理
1. **观察仓位表格**: 实时查看当前仓位状态
2. **关注策略提示**: 根据提示信息决定是否设置交易警报
3. **手动平仓**: 根据自己的交易计划手动平仓
4. **重新开始**: 如需重新交易，修改策略开始时间
