...
**重要说明**：系统的时间区间控制是可选的，用于限制交易时间。

**时间区间控制原理**：
- 默认情况下，不启用时间区间限制，交易可以随时执行
- 如果在交易命令中包含"开启时间区间"，系统会检查当前时间是否在配置的时间段内
- 只有当前时间在配置的自定义时间段内时，交易才会被执行
- 如果不包含"开启时间区间"，则跳过时间检查，直接执行交易
...
**配置方法**：
1. 在TradingView警报中设置Webhook URL: `http://服务器ip/webhook`
2. 选择POST方法
3. 设置消息格式（见第5节）
