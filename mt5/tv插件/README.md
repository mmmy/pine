...
- 🎯 **专门监听新触发的弹窗警报** - 只处理刚刚出现的警报弹窗，不会重复处理已存在的警报
...
3. **查看转发状态**
   - 当警报触发并显示弹窗通知时，插件会自动捕获并转发到你的Webhook URL
   - Console中会显示"🎯 发现新弹窗警报"的日志
   - **重要**: 只有新出现的弹窗警报会被处理，已存在的警报列表不会被重复发送
...
3. **测试弹窗监听功能**
   ```javascript
   // 运行弹窗监听测试
   window.popupMonitorTest.run();

   // 创建测试弹窗警报
   window.popupMonitorTest.createPopup("Test Alert: BTCUSD 50000");
   ```
